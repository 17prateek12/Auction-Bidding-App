...
export const chatWithEvent = async (eventId, question) => {
  const authToken = Cookies.get('authToken'); 
  try {
    const response = await axios.post(
      `${API_URL}getEntireEvent/${eventId}`,
      { question }, 
      {
        headers: {
          Authorization: `Bearer ${authToken}`,
    ...
    console.error('Error in chatting with event:', error.response?.data || error.message);
    throw error.response?.data || error.message;
  }
};